...
from models.models_by_country_mpdisp import (
    ShoppingModelByCountryMPDisp,
    ShoppingSetupModelByCountryMPDisp,
    TotalAssigningModelByCountryMPDisp,
    TransportationModelByCountryMPDisp,
    TransportationSetupModelByCountryMPDisp,
)
from models.models_by_country_single import (
    ShoppingModelByCountrySingle,
    ShoppingSetupModelByCountrySingle,
    TotalAssigningModelByCountrySingle,
    TransportationModelByCountrySingle,
        ...
        if self.multipicking in X.columns:
            ...
            for segment in self.segments:
                if len(X[X[self.multipicking] == segment]) > 0:
                    ...
                    if segment > 0:
                        y_pred[mask] = self.model_mpdisp.predict(
                            X[mask], postprocess=False
                        )
                    else:
                        y_pred[mask] = self.model_single.predict(
                            X[mask], postprocess=False
                        )

            return (
                self.postprocess_predictions(y_pred) if postprocess else y_pred.values
            )
        else:
